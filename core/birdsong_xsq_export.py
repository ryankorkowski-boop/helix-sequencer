...
def emit_birdsong_xsq_sequence(
    *,
    ...
    model_element.set("name", model_name)
    model_element.set("type", "model")
    model_layer = SubElement(model_element, "EffectLayer")
    ...
    for idx, intent in enumerate(ordered):
        ...
        timing_marker.set("label", f"{intent.motif}:{intent.effect_name}")
        timing_marker.set("startTime", _ms(intent.start_time))
        timing_marker.set("endTime", _ms(intent.end_time))

        model_effect = SubElement(model_layer, "Effect")
...
def write_birdsong_xsq(path: Path, intents: Iterable[EffectIntent], *, sequence_name: str = "HelixBirdsongDemo", model_name: str = DEFAULT_BIRDSONG_MODEL) -> Path:
    sequence = emit_birdsong_xsq_sequence(intents=intents, sequence_name=sequence_name, model_name=model_name)
