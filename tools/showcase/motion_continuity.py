"""Report-only showcase motion-continuity metrics.

These metrics operate only on synthetic, internal, or permissioned motion traces.
They do not ingest public videos, copyrighted media, or third-party choreography.
The goal is to measure cinematic motion grammar: directional coherence,
transition smoothness, motion-family persistence, and hard-cut risk.
"""
...
class MotionContinuityReport:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "direction_coherence_score": self.direction_coherence_score,
            "family_continuity_score": self.family_continuity_score,
            "transition_smoothness_score": self.transition_smoothness_score,
            "hard_cut_penalty": self.hard_cut_penalty,
            "showcase_motion_score": self.showcase_motion_score,
...
def score_motion_continuity(raw_motions: Iterable[Mapping[str, object] | MotionTrace]) -> MotionContinuityReport:
    """Score motion continuity without changing generation behavior."""

    motions = normalize_motion_traces(raw_motions)
    findings: list[MotionContinuityFinding] = []
    if not motions:
        finding = MotionContinuityFinding(
            code="no_motion_traces",
            severity="warning",
            message="No motion traces were provided, so motion continuity cannot be measured.",
            penalty=1.0,
        )
        return MotionContinuityReport(
            motion_count=0,
            direction_coherence_score=0.0,
            family_continuity_score=0.0,
            transition_smoothness_score=0.0,
            hard_cut_penalty=1.0,
            showcase_motion_score=0.0,
            findings=(finding,),
        )
    ...
    showcase_motion_score = round(
        max(
            0.0,
            (0.3 * direction_coherence_score)
            + (0.25 * family_continuity_score)
            + (0.25 * transition_smoothness_score)
            + (0.2 * (1.0 - hard_cut_penalty)),
        ),
        4,
    )
    ...
    return MotionContinuityReport(
        ...
        direction_coherence_score=direction_coherence_score,
        family_continuity_score=family_continuity_score,
        transition_smoothness_score=transition_smoothness_score,
        hard_cut_penalty=hard_cut_penalty,
        showcase_motion_score=showcase_motion_score,
        raw_metrics={
            "mean_intensity": round(_mean(motion.intensity for motion in motions), 4),
            "mean_speed": round(_mean(motion.speed for motion in motions), 4),
            "mean_breadth": round(_mean(motion.breadth for motion in motions), 4),
            "unique_motion_families": float(len({motion.motion_family for motion in motions})),
...
def normalize_motion_trace(raw: Mapping[str, object] | MotionTrace) -> MotionTrace:
    if isinstance(raw, MotionTrace):
        return raw
    return MotionTrace(
        name=str(raw.get("name", raw.get("motion_id", "unnamed_motion"))),
        ...
        motion_family=_normalize_token(str(raw.get("motion_family", raw.get("family", raw.get("effect_family", "unknown"))))),
        direction=_normalize_direction(str(raw.get("direction", "none"))),
        intensity=_clamp01(float(raw.get("intensity", 0.5))),
        speed=_clamp01(float(raw.get("speed", raw.get("motion_speed", 0.5)))),
        breadth=_clamp01(float(raw.get("breadth", raw.get("prop_breadth", 0.5)))),
        transition=_normalize_token(str(raw.get("transition", raw.get("transition_type", "cut")))),
    )
...
def _score_family_continuity(motions: list[MotionTrace], findings: list[MotionContinuityFinding]) -> float:
    ...
    for left, right in zip(motions, motions[1:]):
        total += 1
        if left.motion_family == right.motion_family:
            compatible += 1
        elif (left.motion_family, right.motion_family) in COMPATIBLE_MOTION_FAMILIES:
            compatible += 1
        else:
            unrelated_jumps += 1
            if left.intensity >= 0.65 and right.intensity >= 0.65:
                findings.append(
                    MotionContinuityFinding(
                        code="unrelated_high_energy_motion_jump",
                        severity="info",
                        motion=right.name,
                        message=f"High-energy motion '{right.name}' changes to an unrelated motion family.",
                        penalty=0.05,
                    )
                )
...
def _score_transition_smoothness(motions: list[MotionTrace], findings: list[MotionContinuityFinding]) -> float:
    ...
    for motion in motions:
        score = weights.get(motion.transition, 0.55)
        scores.append(score)
        if motion.transition in {"hard_cut", "cut"} and motion.intensity >= 0.75:
            findings.append(
                MotionContinuityFinding(
                    code="high_energy_hard_transition",
                    severity="info",
                    motion=motion.name,
                    message=f"High-energy motion '{motion.name}' uses a hard transition.",
                    penalty=0.05,
                )
            )
...
def _score_hard_cut_penalty(motions: list[MotionTrace], findings: list[MotionContinuityFinding]) -> float:
    ...
    for left, right in zip(motions, motions[1:]):
        ...
        intensity_delta = abs(right.intensity - left.intensity)
        speed_delta = abs(right.speed - left.speed)
        hard_transition = right.transition in {"cut", "hard_cut"}
        if hard_transition and gap <= 0.1 and intensity_delta >= 0.35 and speed_delta >= 0.25:
            risky.append(right)
            findings.append(
                MotionContinuityFinding(
                    code="hard_cut_risk",
                    severity="warning",
                    motion=right.name,
                    message=f"Motion '{right.name}' hard-cuts after a large intensity/speed change.",
                    penalty=0.1,
                )
            )
